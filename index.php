...
$query = "SELECT * FROM produk WHERE nama LIKE '%$pencarian%' LIMIT
$awalData, $dataPerHalaman";
$hasil = mysqli_query($koneksi, $query);
// Query untuk mendapatkan total data tanpa limit
$queryTotal = "SELECT COUNT(*) AS total FROM produk WHERE nama LIKE
'%$pencarian%'";
...
<body>
    ...
    <form method="GET" action="index.php">
        <input type="text" name="pencarian" placeholder="Cari Data..." value="<?=
                                                                                $pencarian; ?>">
    ...
    <div style="margin-top: 10px;">
        <?php if ($halaman > 1): ?>
            <a href="index.php?halaman=<?= $halaman - 1; ?>&pencarian=<?=
                                                                        $pencarian; ?>">Sebelumnya</a>
        <?php endif; ?>
        <?php for ($i = 1; $i <= $totalHalaman; $i++): ?>
            <a href="index.php?halaman=<?= $i; ?>&pencarian=<?= $pencarian; ?>"
                <?= $i == $halaman ? 'style="font-weight: bold;"' : ''; ?>><?= $i; ?></a>
        <?php endfor; ?>
